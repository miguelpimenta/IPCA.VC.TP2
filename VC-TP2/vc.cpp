...
IVC *vc_read_image(char *filename)
{
	...
	if ((file = fopen(filename, "rb")) != NULL)
	{
		...
		if (levels == 1) // PBM
		{
			...
			if (showSteps == 1)
			{
				printf("\nChannels = %d Size %d x %d Levels = %d\n", image->channels, image->width, image->height, levels);
			}
...
int vc_binary_dilate(IVC *src, IVC *dst, int size)
{
	...
	for (y = 0; y < height; y++)
	{
		for (x = 0; x < width; x++)
		{
			...
			for (yvizinhos = xymin; yvizinhos <= xymax; yvizinhos++)
			{
				for (xvizinhos = xymin; xvizinhos <= xymax; xvizinhos++)
				{
					if ((y + yvizinhos >= 0) && (x + xvizinhos >= 0) && (x + xvizinhos < width) && (y + yvizinhos < height))
					{
						posk = (y + yvizinhos) * bytesperline + (x + xvizinhos) * channels;

						if (datasrc[posk] == 0) temp = 0;
					}
				}
			}
			datadst[pos] = temp;
...
int vc_binary_open(IVC *src, IVC *dst, int sizeErode, int size)
{
	...
	vc_binary_erode(src, temp, sizeErode);
	vc_binary_dilate(temp, dst, size);
	vc_image_free(temp);
...
{
	IVC *temp = vc_image_new(src->width, src->height, 1, src->levels);
	vc_binary_dilate(src, temp, size);
	vc_binary_erode(temp, dst, sizeErode);
	vc_image_free(temp);
...
int vc_scale_gray_to_rgb(IVC *src, IVC *dst)
{
	...
	for (d = 0, i = 0; i < 63; i++, d += 4)
	{
		red[i] = 0;
		green[i] = d;
		blue[i] = 255;
	}
	for (d = 256, i = 64; i >= 64 && i < 128; i++, d -= 4)
	{
		red[i] = 0;
		green[i] = 255;
		blue[i] = d;
	}
	for (d = 0, i = 128; i >= 128 && i < 192; i++, d += 4)
	{
		red[i] = d;
		green[i] = 255;
		blue[i] = 0;
	}
	for (d = 256, i = 192; i >= 192 && i < 255; i++, d -= 4)
	{
		red[i] = 255;
		green[i] = d;
		blue[i] = 0;
	...
	//{
	//	datadst[pos_dst] = red[datasrc[pos_src]];
	//	datadst[pos_dst + 1] = green[datasrc[pos_src]];
	//	datadst[pos_dst + 2] = blue[datasrc[pos_src]];
	//}
	for (y = 0; y < height; y++)
	{
		for (x = 0; x < width; x++)
...
int vc_img_to_gray(IVC *srcImg, IVC *dstImgt)
{
	unsigned char *datasrc = (unsigned char *)srcImg->data;
	int bytesperline_src = srcImg->width * srcImg->channels;
	int channels_src = srcImg->channels;

	unsigned char *datadst = (unsigned char *)dstImgt->data;
	int bytesperline_dst = dstImgt->width * dstImgt->channels;
	int channels_dst = dstImgt->channels;
	...
	// verificação de erros
	if ((srcImg->width <= 0) || (srcImg->height <= 0) || (srcImg->data == NULL)) return 0;
	if ((srcImg->width != dstImgt->width) || (srcImg->height != dstImgt->height)) return 0;
	if ((srcImg->channels != 3) || (dstImgt->channels != 1)) return 0;
